...
setInterval(() => {
    http.get(`https://accountshopnew.glitch.me/`);
}, 280000);
...
● | ${prefix}buy sfa/net amount -> لشراء حساب
● | ${prefix}stock -> لمعرفة كمية الحسابات المتوفر
● | ${prefix}give mention sfa/net amount -> لإرسال حساب لشخص
**`);
  }
});
...
let net = JSON.parse(fs.readFileSync('./net.json', 'utf8')); // الملف الي بتحط به الحسابات العاديه
let SFAP = 6000; /*سعر الحساب الواحد الفل داتا*/
let netP = 10000; /*سعر الحساب الواحد العادي*/
let URID = '720350815562367036' //مين بيتحوله الكريديت
client.on('message', async message => { 
    ...
    if (message == prefix + 'stock') {
        ...
        message.channel.send(new Discord.RichEmbed().setTitle('Media shop') ///1
            .addField('**[SFA | فل داتا] > **', `**${ahmed} Account(s)**`, true).addField('**[net | عادي] > **', `**${hossam} Account(s)  **`, true).setColor('GREEN') 
            .addField('**هل تعلم ؟**', `**\`[SFA]\` > فل داتا - حساب يمكنك اللعب وتغيير الاسم والباسوورد والسكن
\`[net]\` > اعتقد انك تعرف نتفليكس**`) 
            .addField('**الاسعار**', `\`[1 SFA] > 6000 Credits ProBot\` \n \`[1 net] > 10000 Credits ProBot\``) 
            .setFooter(`$buy [sfa/net] [ لشراء حساب الرجاء كتابة الأمر التالي [الكمية`))
    } 
...
لديك 3 دقائق قبل الالغاء.**`));
        ...
        if (cmd == 'net') {
            ...
            if (ahmed < args) return message.channel.send("لا يوجد حسابات كافية")
            message.author.send('✅ Nothing.. Just Check If Your DM open or no').then(() => {
                let P = Math.floor(args * (netP)) 
                cools[message.author.id + message.guild.id] = {
                status: "on"
              };
                        let P3 = Math.floor(args * (netP)) 
                message.channel.send(new Discord.RichEmbed().setAuthor(message.author.tag, message.author.avatarURL).setColor('#918383')
                    .setDescription(`**اكتب الامر التالي لأكمال عمليه الشراء
#credits <@${URID}> ${P3}
لديك 3 دقائق قبل الالغاء.**`));
                ...
                message.channel.awaitMessages(filter, {
                    ...
                    .then(collected => { 
                        ...
                        net.forEach(acc => {
                            ...
                            delete acc.email;
                            delete acc.pass;
                            fs.writeFile("./net.json", JSON.stringify(net), (err) => {
                                if (err) console.error(err)
                            })
                        });
                  delete cools [message.author.id + message.guild.id];
...
"pass" : `${pass}`
    ...
    if (message.content.startsWith(prefix + 'monoijo')) {
        ...
        if (type === "net") {
            ...
            message.channel.send('**Done**')
            user.send(`${message.author.username} has been gifted you a net account \n \`\`\`${Accs.join("\n")}\`\`\` `).then(M => M.delete(5 * 60 * 1000))
...
client.login('NzQzODA2MzAxMjg0NDY2NzA4.XzaBmQ.8TSh8zDC5XzE7M2f6WArQHC4gyg');
